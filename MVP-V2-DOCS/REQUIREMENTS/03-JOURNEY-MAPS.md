...
Estado: ALINEADO A FLUJO IMPLEMENTADO + Fase 0 de pipeline formal
...
### Fase 1B - Sales Insight Collector HITL (PARCIAL)
Acciones:
1. Vendedor registra notas libres de reunion (miedos, decisores, presupuesto, urgencia).
2. Backend estructura notas con Gemini a JSON tipado.
3. Insight estructurado se persiste en SQLite (`intel_human_insights`) por `company_id`.
...
3. Habilitar captura de insights humanos en flujo comercial sin romper journey principal.

P1:
1. Mejorar ruta de exportacion formal.
2. Unificar feedback post-propuesta para memoria institucional.
3. Fusionar web intelligence + human insights en `update_summary`.
