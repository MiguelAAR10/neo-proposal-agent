...
**Configuración:**
- Nombre: `neo_cases_v1`
- Dimensión vectores: 768 (`gemini-embedding-001` - *Requisito V1: Nunca usar text-embedding-004 ya que fue depreciado*)
- Distancia: Cosine
...
- Dimensión vectores: 768 (`gemini-embedding-001` - *Mismo requisito de Fallback de V1*)
- Distancia: Cosine
...
**Implementación del Switch en Frontend:**
```text
┌─────────────────────────────────┐
...
**Propósito:** Cachear datos que no cambian frecuentemente (inteligencia de sector, datos de industria) para evitar llamadas repetidas a Gemini.
...
- Manual (Admin): `POST /admin/cache/invalidate?pattern=sector:*`
- Automática: TTL expira automáticamente. Regeneración transparente en próximo request.
...
## Lecciones Críticas Aplicadas desde la V1 (Reglas de Arquitectura)

Para asegurar que la Fase 2 sea exitosa y no repita los bloqueos sufridos durante el desarrollo del MVP (V1), la arquitectura impone las siguientes reglas inquebrantables:

1. **Gestión Estricta de Modelos (Anti-404):** Queda estrictamente prohibido colocar nombres de modelos en duro (*hardcodeados*) en el código (como ocurrió con el extinto `text-embedding-004`). El backend debe leer el modelo (`gemini-embedding-001` o superior) desde las variables de entorno validadas en `src/config.py` y debe implementar lógica de *Fallback* (reintento automático con un modelo de respaldo) si la API de GenAI devuelve un error.
2. **Dependencias "Lazy" (Patrón Lifespan):** Ningún cliente externo (Qdrant, Redis, Google GenAI) debe inicializarse en el nivel global de los scripts. En la V2, **todas** las conexiones deben nacer y morir dentro del bloque asíncrono `@asynccontextmanager async def lifespan(app: FastAPI):` para evitar colapsos en el arranque o dependencias circulares.
3. **Validación Preventiva de Índices en Qdrant:** El Error 500 fatal de la V1 fue causado por buscar en un campo sin índice. En la V2, el *Health Check* de FastAPI debe verificar activamente la existencia de los `Payload Indexes` (`industria`, `area_funcional`) al arrancar. Si los índices no existen, el sistema debe alertar inmediatamente y denegar el arranque, en lugar de fallar silenciosamente ante el usuario.
